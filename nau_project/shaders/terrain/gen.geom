...
void main()
{
    ...
    p[0] = vec4((pos.x    )*scale_x, scale*texture(noise, centers[0] / tesselation).x, (pos.z    )*scale_z, 1);
    p[1] = vec4((pos.x    )*scale_x, scale*texture(noise, centers[1] / tesselation).x, (pos.z + 1)*scale_z, 1);
    p[2] = vec4((pos.x + 1)*scale_x, scale*texture(noise, centers[2] / tesselation).x, (pos.z    )*scale_z, 1);
    p[3] = vec4((pos.x + 1)*scale_x, scale*texture(noise, centers[3] / tesselation).x, (pos.z + 1)*scale_z, 1);

    
    
    for (int i = 0; i < 4; ++i)
    {
        // TODO: THIS MIGHT NOT BE NECESSARY DUE TO TEXTURE REPETITION
